...
**格式 B：Markdown（dbclaw IM 会话，`.md` 后缀）**
...
- **目标文件已存在** → **不直接追加**，改为写临时文件到 `knowledge/.pending/` 目录：
  ...
  - source_id：当前端点标识（SG 端为 `sg-larkbridge`，CN 端为 `cn-dbclaw`）
  - wrapup_id：本次 wrapup 的唯一标识（使用 sessionId 的前 8 位）
  - 时间戳：`YYYYMMDDHHmmss` 格式（UTC）
  - 示例：`feishu-bridge_sg-larkbridge_fdcff362_20260414163000.md`
...
> **注意**：SG 端的 session-end.sh 也有 Git push 兜底逻辑。如果 wrapup 内已经完成了 commit+push，session-end.sh 会检测到没有 dirty 文件而跳过。两者不冲突。
